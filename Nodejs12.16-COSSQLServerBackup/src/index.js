...
const CosSqlserverBackupTask = require('./common/CosSqlserverBackupTask');
const ScfInvokeTask = require('./common/ScfInvokeTask');
const { getParams, logger, getLogSummary } = require('./common/utils');

exports.main_handler = async (event, context) => {
  /**
   * set a timer to terminate the sqlserver backup task, ensure log message is printed
   */
  let runningTask;
  ...
  /**
   * if function is trigger by timer, try to get available backup urls of each sqlserver instance
   * split each backup url with it's sqlserver instance, ensure each task has only one backup url
   * for example, if there are 3 sqlserver instances and each one has 3 backup urls, we will split it into 9 backup tasks
   * it is in order to avoid running timeout
   */
  if (triggerType === 'Timer') {
    const cosSqlserverBackupTask = new CosSqlserverBackupTask({
      secretId,
      secretKey,
      token,
      targetBucket,
    ...
    const splitTaskResultList = await cosSqlserverBackupTask.runBackupSplitTask({
      instanceList,
    });
  ...
  if (triggerType === 'Timer' && backupSplitInstanceList.length > 1) {
    ...
    taskList = backupSplitInstanceList.map(({ instanceId, instanceRegion, instanceStartTime, instanceBackupUrls }) => new CosSqlserverBackupTask({
      secretId,
      secretKey,
      token,
      instanceId,
  ...
  logger({
    title: 'cos sqlserver backup full logs:',
    data: taskResults,
  });

  const { status, messages } = getLogSummary({
    name: 'cos sqlserver backup',
    results: taskResults,
  });
