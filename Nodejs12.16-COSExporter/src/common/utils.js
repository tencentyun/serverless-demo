...
function getParams(event, { function_name }) {
  ...
  try {
    ...
    throw new Error('inputConfig must be JSON Object');
  }
  ...
  try {
    ...
    throw new Error('filterConfig must be JSON Object');
  }

  try {
    if (typeof outputConfig === 'string') {
      outputConfig = JSON.parse(outputConfig);
    }
  } catch (err) {
    throw new Error('outputConfig must be JSON Object');
  }
