...
var ReactAgent = class {
	...
	constructor(options) {
		...
		*/
		const { state, input, output } = require_annotation.createAgentAnnotationConditional(this.options.responseFormat !== void 0, this.options.stateSchema, this.options.middleware);
		const workflow = new __langchain_langgraph.StateGraph({
			state,
			input,
			output
	...
	#createAfterModelRouter(toolClasses, allowJump, exitNode) {
		...
		return (state) => {
			...
			if (allowJump && builtInState.jumpTo) {
				if (builtInState.jumpTo === __langchain_langgraph.END) return exitNode;
				if (builtInState.jumpTo === require_ToolNode.TOOLS_NODE_NAME) {
