...
function getParams(event, { function_name }) {
  ...
  try {
    ...
    if (typeof setAcls === 'string') {
      setAcls = JSON.parse(setAcls);
    }
    if (typeof setTags === 'string') {
      setTags = JSON.parse(setTags);
    }
  } catch (err) {
    throw new Error('setHeaders, setAcls, setTags must be JSON Object');
  }
  ...
  return {
    ...
    setAcls,
    setTags,
    deleteSourceKey: ['true', true].includes(deleteSourceKey),
  };
}
/**
