...
// src/store/tdai.ts
var import_stores = require("@langchain/core/stores");
var TDAIStore = class extends import_stores.BaseStore {
  constructor(fields) {
    super();
...
function convert2LangChain(agkitTool, implFunc) {
  ...
  return (0, import_tools.tool)(func, {
    name: agkitTool.name,
    description: agkitTool.description || agkitTool.name,
    schema: agkitTool.schema || import_v4.z.object({})
  });
}
function convertLangChain2AGKit(langchainTool) {
  const func = async (input, context) => {
    try {
      ...
      return new import_agent_tools.ToolResult({
        success: true,
        data: result
      });
    } catch (error) {
  ...
  return (0, import_agent_tools.tool)(func, {
    name: langchainTool.name,
    description: langchainTool.description || langchainTool.name,
    schema: langchainTool.schema || import_v4.z.object({})
  });
}
...
function clientTools() {
  return (0, import_langchain.createMiddleware)({
    ...
    afterModel: {
      canJumpTo: [import_langgraph.END],
      hook: (state) => {
        const messages = state.messages || [];
        const lastMessage = messages.at(-1);
        if (!import_messages.AIMessage.isInstance(lastMessage)) return;
        ...
        if (anyClientTool) {
          return { jumpTo: import_langgraph.END };
        }
        return;
      }
    }
...
var TDAIChatHistory = class extends import_chat_history.BaseListChatMessageHistory {
  constructor(options) {
    super();
    this.lc_namespace = ["langchain", "stores", "message", "tdai"];
    const { sessionId, ...clientOptions } = options;
    this.client = new import_agent_agents.MemoryClient({
      ...clientOptions
    });
  ...
  async getMessages() {
    ...
    return (0, import_messages2.mapStoredMessagesToChatMessages)(
      events.map((event) => {
        return {
          type: event.type,
          data: event.data
  ...
  async addMessage(message) {
    const messages = (0, import_messages2.mapChatMessagesToStoredMessages)([message]);
