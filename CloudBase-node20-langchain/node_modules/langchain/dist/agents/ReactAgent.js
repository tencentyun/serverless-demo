...
var ReactAgent = class {
	...
	constructor(options) {
		...
		*/
		const { state, input, output } = createAgentAnnotationConditional(this.options.responseFormat !== void 0, this.options.stateSchema, this.options.middleware);
	...
	#createAfterModelRouter(toolClasses, allowJump, exitNode) {
		...
		return (state) => {
			...
			if (allowJump && builtInState.jumpTo) {
				if (builtInState.jumpTo === END) return exitNode;
