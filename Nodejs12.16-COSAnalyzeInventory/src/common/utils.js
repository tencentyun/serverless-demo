...
function getParams(event, context) {
  ...
  try {
    if (typeof preAnalyzeConfig === 'string') {
      preAnalyzeConfig = JSON.parse(preAnalyzeConfig);
    }
  } catch (err) {
    throw new Error('preAnalyzeConfig must be JSON Object');
  }

  try {
    if (typeof analyzeConfig === 'string') {
      analyzeConfig = JSON.parse(analyzeConfig);
    }
  } catch (err) {
    throw new Error('analyzeConfig must be JSON Object');
  }
