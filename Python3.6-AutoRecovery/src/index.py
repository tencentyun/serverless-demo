...
def run_tat_command(timestamp, date, secretid, secretkey, region, insid, work_directory, user, run_content):
    ...
    action = "RunCommand"
    version = "2020-10-28"
    payload = "{\"CommandName\":\"cmd-test\",\"CommandType\":\"SHELL\",\"InstanceIds\":[\"%s\"],\"WorkingDirectory\":\"%s\",\"Username\":\"%s\",\"Content\":\"%s\"}" % (
    insid, work_directory, user, run_content)
    params = json.loads(payload)
    algorithm = "TC3-HMAC-SHA256"
...
def main_handler(event, context):
    ...
    while loop <= 6:
        if status == 'RUNNING':
            # 服务器命令执行
            time.sleep(10)
            tat_command = run_tat_command(timestamp, date, secretid, secretkey, region, insid, work_directory, user,
                                          run_command_base.decode('utf-8'))
