...
function isLegal(encoding) {
  return (
    LEGAL_ENCODING_LIST.includes(encoding)
 ...
 */
function getDetectResults(originBuffer) {
  const results = [];
  const detector = new UniversalDetector();
  const buffer = expandBuffer({ buffer: originBuffer, minSize: 300 });
  detector.reset();
  ...
  const { confidence } = detector.result || {};
  let { encoding } = detector.result || {};
