...
function getParams({
  ...
  const missingParams = [
    { key: 'domainList', value: domainListStr },
  ...
  let domainList = domainListStr;

  try {
    domainList = JSON.parse(domainListStr);
  } catch (err) {
    throw new Error('params parsed error, domainList should be a json array string');
  }
