...
def consumer_worker(event, context, start_time):
    ...
    kafka_to_cos = KafkaToCos(start_time, request_id, topic_name, kafka_address, bucket_address,
...
class KafkaToCos(object):
    ...
    def __init__(self, start_time, request_id, topic_name, kafka_address, bucket_address,
                 file_path_prefix, once_max_to_cos_bytes, function_timeout_ms, partition_id,
    ...
    # Generating file name. 生成写入文件名, 格式为kafka_addr/topic_name/date/timestamp_request_id
    def object_key_generate(self):
        ...
        if str(self.file_path_prefix) == "":
            dir_name = "{}/{}/{}".format(str(self.kafka_address), str(self.topic_name), today)
        else:
            dir_name = "{}/{}/{}/{}".format(str(self.file_path_prefix), str(self.kafka_address), str(self.topic_name),
                                            today)
