...
async function initializeMiddlewareStates(middlewareList, state) {
	...
	for (const middleware of middlewareList) {
		...
		*/
		...
		if (__langchain_langgraph.StateSchema.isInstance(middleware.stateSchema)) {
			const zodShape = {};
			for (const [key, field] of Object.entries(middleware.stateSchema.fields)) if (__langchain_langgraph.ReducedValue.isInstance(field)) zodShape[key] = field.inputSchema || field.valueSchema;
			else zodShape[key] = field;
			zodSchema = zod_v4.z.object(zodShape);
		}
		const modifiedSchema = (0, __langchain_core_utils_types.interopZodObjectMakeFieldsOptional)(zodSchema, (key) => key.startsWith("_"));
...
function derivePrivateState(stateSchema) {
	...
	if (__langchain_langgraph.StateSchema.isInstance(stateSchema)) {
		shape = {};
		for (const [key, field] of Object.entries(stateSchema.fields)) if (__langchain_langgraph.ReducedValue.isInstance(field)) shape[key] = field.inputSchema || field.valueSchema;
