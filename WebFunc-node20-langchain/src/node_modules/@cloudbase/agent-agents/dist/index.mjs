import {
  ...
  __export,
  __toCommonJS
} from "./chunk-NYRGWMQB.mjs";

// src/core/events.ts
import { EventEmitter } from "events";
var EventType, EventSystem;
var init_events = __esm({
  "src/core/events.ts"() {
    ...
    EventSystem = class extends EventEmitter {
      constructor() {
        super();
...
// src/agkit/agent.ts
import { AbstractAgent, EventType as EventType2 } from "@ag-ui/client";
...
var init_agent = __esm({
  "src/agkit/agent.ts"() {
    "use strict";
    init_events();
    AGKitAgent = class extends AbstractAgent {
      ...
      async _run(subscriber, input) {
        ...
        try {
          subscriber.next({ type: EventType2.RUN_STARTED, threadId, runId });
...
import { AbstractAgent as AbstractAgent2 } from "@ag-ui/client";
function toAGUIAgent(agent, config) {
  if (agent instanceof AbstractAgent2) {
    return agent;
  }
...
var HumanInTheLoopConfigSchema = z.object({
  enabled: z.boolean(),
...
var ControlFlowConfigSchema = z.object({
...
var AgentExecutor = class {
    ...
    for (const toolCallDelta of toolCallDeltas) {
      ...
      if (!toolCalls[toolCallIndex]) {
        toolCalls[toolCallIndex] = {
          id: toolCallDelta.id || randomUUID3(),
  ...
  prepareToolDefinitions(options) {
    ...
    return mergedTools.map((tool2) => {
      ...
      if (isZodSchema) {
        try {
          parameters = z2.toJSONSchema(tool2.schema);
        } catch (error) {
...
var Agent = class {
  ...
  async run(input, state, options) {
    ...
    const runId = validatedOptions?.runId || randomUUID4();
    const conversationId = options?.conversationId || randomUUID4();
    const eventEmitter = {
      emit: (type, event) => {
  ...
  async *stream(input, state, options) {
    const runId = options?.runId || randomUUID4();
    const conversationId = options?.conversationId || randomUUID4();
    const eventQueue = [];
    let isDone = false;
    let error = null;
...
var StateConstraintSchema = z3.record(z3.string(), z3.unknown());
var OutputConstraintSchema = z3.union([
  z3.string(),
...
var OptimizedEventSystem = class extends EventEmitter2 {
  constructor(options = {}) {
    super();
    this.eventQueue = [];
...
function createTool(name, description, handler, options) {
  ...
  return tool(handler, {
    name,
    description,
    schema: options?.schema || z4.object({}),
...
__decorateClass([
  Column2({ type: "varchar", length: 500, default: "main" })
], MemoryState.prototype, "currentBranch", 2);
__decorateClass([
  Column2({ type: "datetime", nullable: true })
...
MemoryState = __decorateClass([
  Entity2("memory_state"),
  Index2("IDX_STATE_SESSION", ["sessionId"])
], MemoryState);
...
__decorateClass([
  Column3({ type: "varchar", length: 255 })
...
MemorySummary = __decorateClass([
  Entity3("memory_summaries"),
  Index3("IDX_SUMMARY_SESSION", ["sessionId"]),
  Index3("IDX_SUMMARY_ID", ["sessionId", "summaryId"])
], MemorySummary);
...
var MySQLMemory = class _MySQLMemory extends TypeORMMemory {
  ...
  static createMySQLDataSource(config) {
    ...
    return new DataSource2(options);
  }
  /**
...
import { DataSource as DataSource4, Repository } from "typeorm";
