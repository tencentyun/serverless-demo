...
const LLMToolSelectorOptionsSchema = zod_v3.z.object({
	model: zod_v3.z.string().or(zod_v3.z.instanceof(__langchain_core_language_models_base.BaseLanguageModel)).optional(),
...
IMPORTANT: List the tool names in order of relevance, with the most relevant first. If you exceed the maximum number of tools, only the first ${maxTools} will be used.`;
	...
	*/
	let lastUserMessage;
	for (const message of request.messages) if (__langchain_core_messages.HumanMessage.isInstance(message)) lastUserMessage = message;
